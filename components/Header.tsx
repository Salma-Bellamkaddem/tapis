...
// Sélecteur de devises intégré
function CurrencySelector() {
  const { currency, setCurrency } = useCurrency();

  return (
    <div className="flex items-center gap-1 bg-[#FAF0E4] border border-[#A44E36]/30 px-2.5 py-1 rounded-full text-xs font-bold text-[#A44E36]">
      <span className="text-[10px] uppercase opacity-75 mr-1 hidden lg:inline">Currency:</span>
      {(["USD", "EUR", "MAD"] as const).map((curr) => (
        <button
          key={curr}
  ...
  return (
    <header className="w-full sticky top-0 z-50 shadow-sm">
      ...
      <nav className="bg-[#FAF9F6] border-b border-gray-200 px-4 md:px-8 py-4 flex justify-between items-center relative">
        ...
        {/* Actions (Sélecteur de devises, WhatsApp & Bouton Burger Mobile) */}
        <div className="flex items-center gap-3">
          
          {/* Sélecteur de devises intégré ici */}
          <CurrencySelector />

          <a 
            href="https://wa.me/212767149114?text=Hello,%20I%20would%20like%20to%20know%20more%20about%20your%20Berber%20rugs."
            target="_blank" 
            rel="noreferrer"
          ...
          <button 
            ...
            className="lg:hidden p-2 text-gray-800 hover:text-[#A44E36] focus:outline-none"
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden bg-[#FAF9F6] border-b border-gray-200 px-6 py-6 shadow-lg animate-fadeIn">
